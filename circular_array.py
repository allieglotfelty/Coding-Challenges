...
class CircularArray(object):
    ...
    def add_item(self, item):
        ...
        self._circ_array.append(item)

    def get_by_index(self, index):
        """Return the data at a particular index."""

        if index > (len(self._circ_array) - 1):
            return None
        return self._circ_array[index]
    ...
    def rotate(self, increment):
        ...
        increment = increment % len(self._circ_array)
        rotated_array = [None] * len(self._circ_array)

        for index, item in enumerate(self._circ_array):
            rotated_array[index - increment] = item
        self._circ_array = rotated_array


    def print_array(self):
        """Print the circular array items in order, one per line"""

        for item in self._circ_array:
            print item
