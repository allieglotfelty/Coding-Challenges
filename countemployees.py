...
class Node(object):
    ...
    def __repr__(self):
        ...
        return '<%s children: %s>' % (self.name, self.children)
    ...
    def count_employees_nonrecursive(self):
        ...
        count = 0
        people_to_count = self.children

        while people_to_count:
            counted = people_to_count.pop(0)
            people_to_count.extend(counted.children)
            count += 1
