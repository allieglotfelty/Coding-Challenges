...
class FriendGraph(object):
    ...
    def are_connected(self, name1, name2):
        ...
        seen = set()
        seen.add(person1)
        people_to_check = list(person1.adjacent)

        while people_to_check:
            person_to_check = people_to_check.pop(0)
            if person_to_check.name == name2:
                return True
            seen.add(person_to_check)
            for person in person_to_check.adjacent:
